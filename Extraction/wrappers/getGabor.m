function gbFeature = getGabor(image, lambda, theta)

[gb_c, gb_s] = gabor2dkernels(theta, lambda, 1, 0, 1);

gbKernel = sqrt(gb_c.^2 + gb_s.^2); % combined response

if (ndims(image) == 2 || ndims(image) == 3)
        for z = 1 : size(image, 3)
            gbFeature(:, :, z) = conv2(im2double(image(:, :, z)), gbKernel, 'same');
            %gbFeature(:, :, z) = rescale(gbFeature(:, :, z));
        end
...
function [gb_c,gb_s]=gabor2dkernels(theta,lambda,b,psi,gamma)
