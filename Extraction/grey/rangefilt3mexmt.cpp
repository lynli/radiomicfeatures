...
#ifdef _WIN32
unsigned __stdcall rangefilt3(void *ThreadArgsV) {
#else
void *rangefilt3(void *ThreadArgsV) {
#endif
// void rangefilt3(double ***volmat, double *medvals, int *ws, 
//         int rows, int cols, int slices) {
    
    double *blockvals, minval, maxval;
    ...
    rangefilt3_threadargs *ThreadArgs=(rangefilt3_threadargs *) ThreadArgsV;
    
    double ***volmat, *rangevals;
    int *ws, rows, cols, slices;
    size_t ThreadOffset, Nthreads;
    
    volmat=ThreadArgs->volmat;
    rangevals=ThreadArgs->rangevals;
    ws=ThreadArgs->ws;
    ...
    for (k = 0; k<slices; ++k) {
        for (j = ThreadOffset; j<cols; j+=Nthreads, ii+=rows*(Nthreads-1)) {
            for (i = 0; i<rows; ++i, ++ii) {
                ...
                for (bvi=1, maxval=minval=*blockvals; bvi<blen; ++bvi) {
                    if (blockvals[bvi]<minval)
                        minval=blockvals[bvi];
                    else if (blockvals[bvi]>maxval)
                        maxval=blockvals[bvi];
                }
                
                rangevals[ii]=maxval-minval;
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    double *vol, ***volmat, *rangevals, *wsd;
    ...
    if(nrhs > 2 || nrhs < 1)
        mexErrMsgTxt("rangefilt3mexmt(volume,ws)");
...
//     meddims[0] = rows; meddims[1] = cols; meddims[2] = slices;
    ...
    rangevals = mxGetPr(plhs[0]);
    
    // Threading
    size_t *ThreadID;
    rangefilt3_threadargs **ThreadArgs;
    rangefilt3_threadargs *ThreadArgsi;
...
//     if (Nthreads>nmaxthreads) Nthreads=nmaxthreads; // input thread limit
	...
	ThreadArgs = (rangefilt3_threadargs **) malloc(Nthreads*sizeof(rangefilt3_threadargs *));
    ...
    for (i=0; i<Nthreads; i++) {
        ...
        ThreadArgsi = (rangefilt3_threadargs *) malloc(sizeof(rangefilt3_threadargs));
        ThreadArgsi->volmat=volmat;
        ThreadArgsi->rangevals=rangevals;
        ThreadArgsi->ws=ws;
        ...
        #ifdef _WIN32
            ThreadList[i] = (HANDLE)_beginthreadex( NULL, 0, &rangefilt3, ThreadArgs[i] , 0, NULL );
        #else
            pthread_create((pthread_t*)&ThreadList[i], NULL, &rangefilt3, ThreadArgs[i]);
        #endif
    }
    ...
    // rangefilt3(volmat,rangevals,ws,rows,cols,slices);
}
